...
DOUGH_NOT_FOUND = 'Dough with ID {} not found.'
...
def get_all_doughs(db: Session = Depends(get_db)):
    ...
    logging.info(f'Found {len(doughs)} doughs')
    return doughs
...
def create_dough(dough: DoughCreateSchema,
    ...
    if dough_found:
        ...
        logging.warning('Created dough already exists under Path: {}'.format(url))
...
def update_dough(
    ...
    if dough_found:
        if dough_found.name == changed_dough.name:
            ...
            logging.info('Dough {} updated successfully from price {} to {}, description {} to {} and stock {} to {}'
                         .format(changed_dough.name,
                                 dough_found.price, changed_dough.price,
                                 dough_found.description, changed_dough.description,
                                 dough_found.stock, changed_dough.stock))
            dough_crud.update_dough(dough_found, changed_dough, db)
            ...
            if dough_name_found:
                ...
                logging.warning('Dough with id {} already exists under Path: {} with name {} not {}'
                                .format(dough_id, url, dough_name_found.name, changed_dough.name))
                return RedirectResponse(url=url, status_code=status.HTTP_303_SEE_OTHER)
            else:
                logging.warning("Dough with id {} didn't exist".format(dough_id))
                updated_dough = dough_crud.create_dough(changed_dough, db)
                response.status_code = status.HTTP_201_CREATED
    else:
        logging.error(DOUGH_NOT_FOUND.format(dough_id))
...
def get_dough(dough_id: uuid.UUID,
    ...
    if not dough:
        logging.error(DOUGH_NOT_FOUND.format(dough_id))
        raise HTTPException(status_code=404)
    logging.info('Dough {} found'.format(dough_id))
    return dough
...
def delete_dough(dough_id: uuid.UUID, db: Session = Depends(get_db)):
    ...
    if not dough:
        logging.error(DOUGH_NOT_FOUND.format(dough_id))
        raise HTTPException(status_code=404)
    logging.info('Dough {} deleted'.format(dough_id))
