...
def update_address(address: Address, changed_address: AddressCreateSchema, db: Session):
    ...
    logging.info(
        ...
        'first_name={}, '
        'last_name={}'.format(
            changed_address.street,
            changed_address.post_code,
            changed_address.house_number,
            changed_address.country,
            changed_address.town,
            changed_address.first_name,
            changed_address.last_name
        )
    )
    db.refresh(address)
    return address
