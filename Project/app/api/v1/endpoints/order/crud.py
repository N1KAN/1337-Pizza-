...
def create_order(schema: OrderCreateSchema, db: Session):
    ...
    db.commit()
    logging.info(
        'Order created: user_id={}, order_status={}, id={}, order_datetime={}'.format(
            order.user_id, order.order_status, order.id, order.order_datetime
        )
    )
    return order
...
def add_pizza_to_order(order: Order, pizza_type: PizzaType,
    ...
    db.refresh(order)
    logging.info(
        'Added pizza to order: pizza_type_id={}'.format(
            pizza.pizza_type_id
        )
    )
    return pizza
...
def create_beverage_quantity(
    ...
    db.refresh(order)
    logging.info(
        'Created order beverage: quantity={}, beverage_id={}'.format(
            entity.quantity , entity.beverage_id
        )
    )
    return entity
...
def update_beverage_quantity_of_order(order_id: uuid.UUID, beverage_id: uuid.UUID, new_quantity: int, db: Session):
    ...
    if order_beverage:
        ...
        logging.info(
            'Updated order beverage: quantity={}, beverage_id={}'.format(
                order_beverage.quantity, order_beverage.beverage_id
            )
        )
