...
from app.api.v1.endpoints.sauces.schemas import SauceSpicienessEnum
...
class Sauce(Base):
    ...
    description: Mapped[str] = mapped_column(nullable=False, default='')
    stock: Mapped[int] = mapped_column(CheckConstraint('stock >= 0'), nullable=False)  # NOSONAR
    spicieness: Mapped[SauceSpicienessEnum] = mapped_column(SqlEnum(SauceSpicienessEnum),
                                                            nullable=True, default=NULLTYPE)

    def __repr__(self):
        return (f"Sauces(id='{self.id}', name='{self.name}', price='{self.price}', description='{self.description}',"
                f" stock='{self.stock}', spicieness='{self.spicieness}')")
...
class User(Base):
    ...
    customer_orders: Mapped[List['Order']] = relationship(cascade='all, delete-orphan', back_populates='user') #NOSONAR
